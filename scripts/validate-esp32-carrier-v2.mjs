...
const expectedSha =
    "d76be0c4625d4a34c3e5a09d863da20c15aa4491002270aa1ff017080ce78dda";
const actualSha = crypto.createHash("sha256").update(text).digest("hex");
if (actualSha !== expectedSha)
    failures.push(`fixture SHA mismatch: ${actualSha}`);
if (!text.startsWith("(kicad_pcb"))
    failures.push("not a KiCad PCB S-expression");
...
for (const name of [
    ...
    requireText(name);
for (const jp of ["JP_RBRK", "JP_LBRK", "JP_LREV", "JP_RREV"])
    requireText(`"${jp}"`, jp);
if (count(/\(property "Value" "2N7002"/g) !== 4)
    failures.push("expected exactly four 2N7002 output MOSFETs");

// Sensor protection must be exactly 10k over 20k on all six Hall and three ECHO inputs.
if (count(/\(property "Value" "10k"/g) < 9)
    failures.push("expected at least nine 10k divider resistors");
if (count(/\(property "Value" "20k"/g) !== 9)
    failures.push("expected exactly nine 20k divider resistors");
if (count(/\(property "Value" "18k"/g) !== 0)
    failures.push("18k divider value must not remain");
for (const name of [
    "L_HALL_A_RAW",
    "L_HALL_B_RAW",
    "L_HALL_C_RAW",
...
for (const ref of ["J_RGB_5V", "J_RGB_DATA", "J_RGB_GND"])
    requireText(`"${ref}"`, ref);
...
for (const label of [
    ...
    "RIGHT BRAKE / GPIO32",
    "LEFT BRAKE / GPIO33",
    "LEFT THROTTLE / GPIO25",
    "RIGHT THROTTLE / GPIO26",
    "LEFT REVERSE / GPIO27",
    "RIGHT REVERSE / GPIO14",
    "3V3 RAIL 1",
    "3V3 RAIL 4",

...
let board = zlib
    .gunzipSync(Buffer.from(fs.readFileSync(SOURCE, "utf8").trim(), "base64"))
    .toString("utf8");
...
function lt8645s() {
    ...
    const net = (p) =>
        nets.has(p) ? ` (net ${nets.get(p)[0]} "${nets.get(p)[1]}")` : "";
    const pads = [];
    const ys = [
...
function via(x, y, net, name, size = 1.0, drill = 0.5) {
    return `(via (at ${x} ${y}) (size ${size}) (drill ${drill}) (layers "F.Cu" "B.Cu") (net ${net} "${name}"))`;
}
function seg(x1, y1, x2, y2, w, layer, net, name) {
    return `(segment (start ${x1} ${y1}) (end ${x2} ${y2}) (width ${w}) (layer "${layer}") (net ${net} "${name}"))`;
}
...
const routes = [
    ...
    seg(22.9, 16.2, 22.9, 18.6, 0.25, "F.Cu", 4, "L_HALL_C_GPIO"),
    seg(25.1, 16.2, 25.1, 18.6, 0.35, "F.Cu", 3, "GND"),
    seg(93.1, 30.2, 93.1, 32.5, 0.25, "F.Cu", 19, "L_HALL_A_GPIO"),
    seg(90.9, 30.2, 90.9, 32.5, 0.35, "F.Cu", 3, "GND"),
    seg(93.1, 36.2, 93.1, 38.5, 0.25, "F.Cu", 20, "L_HALL_B_GPIO"),
    seg(90.9, 36.2, 90.9, 38.5, 0.35, "F.Cu", 3, "GND"),
    seg(93.1, 54.2, 93.1, 56.5, 0.25, "F.Cu", 23, "R_HALL_A_GPIO"),
    seg(90.9, 54.2, 90.9, 56.5, 0.35, "F.Cu", 3, "GND"),
    seg(93.1, 60.2, 93.1, 62.5, 0.25, "F.Cu", 24, "R_HALL_B_GPIO"),
    seg(90.9, 60.2, 90.9, 62.5, 0.35, "F.Cu", 3, "GND"),
    seg(93.1, 78.2, 93.1, 80.5, 0.25, "F.Cu", 27, "R_HALL_C_GPIO"),
    seg(90.9, 78.2, 90.9, 80.5, 0.35, "F.Cu", 3, "GND"),
    ...
    seg(59.62, 118, 59.62, 128, 1.2, "F.Cu", 57, "VIN_PROTECTED"),
    ...
    seg(8, 80, 8, 86, 0.3, "F.Cu", 15, "SERVO_SIG"),
    seg(10.54, 86, 19.8, 90, 0.8, "F.Cu", 64, "+5V_SERVO"),
    seg(19.8, 90, 24, 94, 0.8, "F.Cu", 64, "+5V_SERVO"),
    seg(16.2, 90, 14, 90, 0.8, "F.Cu", 2, "+5V"),
    seg(14, 90, 14, 92, 0.8, "F.Cu", 2, "+5V"),
];
...
insertBefore(
    "(zone",
    [...routes, ...localGround, ...vias, ...graphics, ...zones].join("\n  "),
);

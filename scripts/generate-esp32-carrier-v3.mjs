...
function addNets(entries) {
  ...
  if (lastEnd < 0) throw new Error('net declaration insertion point not found');
  const declarations = entries.map(([id, name]) => `\n  (net ${id} "${name}")`).join('');
...
function insertRouting(text) {
  ...
  else board = board.slice(0, board.lastIndexOf(')')) + '\n  ' + text + '\n' + board.slice(board.lastIndexOf(')'));
}

function passive({ref, value, x, y, rot = 0, net1, name1, net2, name2, kind = 'C', size = '0805'}) {
...
function terminal2({ref, value, x, y, p1net, p1name, p2net = 3, p2name = 'GND'}) {
...
function lt8645s() {
  const nets = new Map([
    [1, [2, '+5V']], [4, [55, 'VIN_PROTECTED']], [5, [55, 'VIN_PROTECTED']], [6, [55, 'VIN_PROTECTED']],
    [8, [3, 'GND']], [9, [3, 'GND']], [10, [3, 'GND']],
    [12, [56, 'PWR_SW']], [13, [56, 'PWR_SW']], [14, [56, 'PWR_SW']], [15, [56, 'PWR_SW']], [16, [56, 'PWR_SW']],
    [17, [3, 'GND']], [18, [3, 'GND']], [19, [3, 'GND']],
    [21, [55, 'VIN_PROTECTED']], [22, [55, 'VIN_PROTECTED']], [23, [55, 'VIN_PROTECTED']],
    [25, [58, 'PWR_EN']], [26, [59, 'PWR_RT']], [28, [3, 'GND']], [29, [61, 'PWR_SS']], [30, [3, 'GND']], [32, [57, 'PWR_FB']],
    [33, [3, 'GND']], [34, [3, 'GND']], [35, [3, 'GND']], [36, [3, 'GND']], [37, [3, 'GND']], [38, [3, 'GND']],
  ]);
  const padNet = (pin) => nets.has(pin) ? ` (net ${nets.get(pin)[0]} "${nets.get(pin)[1]}")` : '';
  const pads = [];
  const ys = [-2.25,-1.75,-1.25,-0.75,-0.25,0.25,0.75,1.25,1.75,2.25];
  for (let i = 0; i < 10; i++) pads.push(`    (pad "${i + 1}" smd roundrect (at -2.45 ${ys[i]}) (size 1.15 0.38) (layers "F.Cu" "F.Paste" "F.Mask") (roundrect_rratio 0.2)${padNet(i + 1)})`);
  const xsBottom = [-1.25,-0.75,-0.25,0.25,0.75,1.25];
  for (let i = 0; i < 6; i++) pads.push(`    (pad "${11 + i}" smd roundrect (at ${xsBottom[i]} 3.45) (size 0.38 1.15) (layers "F.Cu" "F.Paste" "F.Mask") (roundrect_rratio 0.2)${padNet(11 + i)})`);
  for (let i = 0; i < 10; i++) pads.push(`    (pad "${17 + i}" smd roundrect (at 2.45 ${ys[9 - i]}) (size 1.15 0.38) (layers "F.Cu" "F.Paste" "F.Mask") (roundrect_rratio 0.2)${padNet(17 + i)})`);
  const xsTop = [1.25,0.75,0.25,-0.25,-0.75,-1.25];
  for (let i = 0; i < 6; i++) pads.push(`    (pad "${27 + i}" smd roundrect (at ${xsTop[i]} -3.45) (size 0.38 1.15) (layers "F.Cu" "F.Paste" "F.Mask") (roundrect_rratio 0.2)${padNet(27 + i)})`);
  const ep = [[33,-0.7,-1.75],[34,0.7,-1.75],[35,-0.7,0],[36,0.7,0],[37,-0.7,1.75],[38,0.7,1.75]];
  for (const [pin,x,y] of ep) pads.push(`    (pad "${pin}" smd rect (at ${x} ${y}) (size 1.25 1.45) (layers "F.Cu" "F.Paste" "F.Mask")${padNet(pin)})`);
...
board = board.replace('(gr_rect (start 1 1) (end 129 109)', '(gr_rect (start 1 1) (end 129 139)');

addNets([
  [53,'BAT_RAW'], [54,'BAT_FUSED'], [55,'VIN_PROTECTED'], [56,'PWR_SW'], [57,'PWR_FB'],
  [58,'PWR_EN'], [59,'PWR_RT'], [60,'+5V_SERVO'], [61,'PWR_SS'],
]);

const hallCaps = [
  passive({ref:'C4', value:'10nF', x:24, y:18.6, net1:4, name1:'L_HALL_C_GPIO', net2:3, name2:'GND'}),
  passive({ref:'C5', value:'10nF', x:92, y:32.5, rot:180, net1:19, name1:'L_HALL_A_GPIO', net2:3, name2:'GND'}),
  passive({ref:'C6', value:'10nF', x:92, y:38.5, rot:180, net1:20, name1:'L_HALL_B_GPIO', net2:3, name2:'GND'}),
  passive({ref:'C7', value:'10nF', x:92, y:56.5, rot:180, net1:23, name1:'R_HALL_A_GPIO', net2:3, name2:'GND'}),
  passive({ref:'C8', value:'10nF', x:92, y:62.5, rot:180, net1:24, name1:'R_HALL_B_GPIO', net2:3, name2:'GND'}),
  passive({ref:'C9', value:'10nF', x:92, y:80.5, rot:180, net1:27, name1:'R_HALL_C_GPIO', net2:3, name2:'GND'}),
];

const powerParts = [
  terminal2({ref:'J_BAT', value:'36-42V_BATTERY', x:8, y:120, p1net:53, p1name:'BAT_RAW'}),
  fuse2410({x:22, y:120}),
  diodeSMC({ref:'D1', value:'SS510_5A_100V_REVERSE_PROTECT', x:34, y:120, net1:54, name1:'BAT_FUSED', net2:55, name2:'VIN_PROTECTED'}),
  diodeSMC({ref:'D2', value:'SMCJ43A_TVS', x:45, y:128, rot:90, net1:55, name1:'VIN_PROTECTED', net2:3, name2:'GND'}),
  radialCap({ref:'C10', value:'100uF_63V_INPUT_BULK', x:49, y:133, net1:55, name1:'VIN_PROTECTED'}),
  passive({ref:'C11', value:'4.7uF_100V_X7R', x:54, y:121.5, net1:55, name1:'VIN_PROTECTED', net2:3, name2:'GND', size:'1206'}),
  passive({ref:'C12', value:'0.47uF_100V_X7R', x:60, y:121.5, net1:55, name1:'VIN_PROTECTED', net2:3, name2:'GND'}),
  lt8645s(),
  passive({ref:'C13', value:'0.47uF_100V_X7R', x:72, y:121.5, rot:180, net1:55, name1:'VIN_PROTECTED', net2:3, name2:'GND'}),
  passive({ref:'R31', value:'100k_EN', x:73, y:116.5, net1:55, name1:'VIN_PROTECTED', net2:58, name2:'PWR_EN', kind:'R'}),
  passive({ref:'R32', value:'88.7k_500kHz', x:77, y:119.5, rot:90, net1:59, name1:'PWR_RT', net2:3, name2:'GND', kind:'R'}),
  passive({ref:'C14', value:'0.1uF_SOFTSTART', x:80, y:119.5, rot:90, net1:61, name1:'PWR_SS', net2:3, name2:'GND'}),
  passive({ref:'R33', value:'1.06M_FB_TOP', x:60, y:115.5, net1:2, name1:'+5V', net2:57, name2:'PWR_FB', kind:'R'}),
  passive({ref:'R34', value:'249k_FB_BOTTOM', x:66, y:115.5, net1:57, name1:'PWR_FB', net2:3, name2:'GND', kind:'R'}),
  passive({ref:'C15', value:'2.2pF_FF', x:63, y:112.8, net1:2, name1:'+5V', net2:57, name2:'PWR_FB'}),
  inductor(),
  radialCap({ref:'C16', value:'100uF_10V_OUTPUT_BULK', x:75, y:132, net1:2, name1:'+5V'}),
  passive({ref:'C17', value:'22uF_10V_X7R', x:84, y:132, rot:90, net1:2, name1:'+5V', net2:3, name2:'GND', size:'1206'}),
  passive({ref:'C18', value:'22uF_10V_X7R', x:90, y:132, rot:90, net1:2, name1:'+5V', net2:3, name2:'GND', size:'1206'}),
  piConnector(),
  mountingHole('H5', 5, 135), mountingHole('H6', 124, 135),
];

const servoParts = [
  servoHeader(),
  passive({ref:'FB2', value:'FERRITE_5A_SERVO', x:15, y:82.54, net1:2, name1:'+5V', net2:60, name2:'+5V_SERVO', kind:'FB', size:'1206'}),
  radialCap({ref:'C19', value:'470uF_10V_SERVO_BULK', x:21, y:84, net1:60, name1:'+5V_SERVO'}),
];
...
const vias = [
  ...
  [65.3,119.75],[66.7,119.75],[65.3,121.5],[66.7,121.5],[65.3,123.25],[66.7,123.25],
  // Ground stitching around input/output capacitors and regulator.
  [55.4,121.5],[73.4,121.5],[78.4,119.5],[81.4,119.5],[78.5,132],[84,133.4],[90,133.4],[48.4,128],
].map(([x,y]) => `(via (at ${x} ${y}) (size 1.2) (drill 0.6) (layers "F.Cu" "B.Cu") (net 3))`);

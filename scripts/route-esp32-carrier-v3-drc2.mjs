...
execFileSync(process.execPath, ["scripts/route-esp32-carrier-v3-clean.mjs"], {
    stdio: "inherit",
});
...
function coordYs(text) {
    return [...text.matchAll(/\((?:start|end|at)\s+[-\d.]+\s+([-\d.]+)/g)].map(
        (m) => Number(m[1]),
    );
}
...
for (const token of ["(segment", "(via"]) {
    for (const entry of blocks(token)) {
        ...
        const isCustomPowerOrGround =
            (net === 2 || net === 3) && ys.some((y) => y >= 110);
        const isCustomServo =
            net === 15 &&
            (entry.text.includes("(start 8 80)") ||
                entry.text.includes("(start 5 ") ||
                entry.text.includes("(at 5 "));
        if (isNewNet || isCustomPowerOrGround || isCustomServo)
...
r.push(
    ...
    seg(23.6, 113.5, 23.6, 126, 0.7, "F.Cu", 54),
    seg(34.8, 113.5, 34.8, 119.025, 0.3, "F.Cu", 54),
    via(45, 113.5, 54),
    seg(45, 113.5, 45, 131.0, 0.3, "B.Cu", 54),
    seg(45, 131.0, 48.5, 131.0, 0.3, "B.Cu", 54),
    via(48.5, 131.0, 54),
    seg(48.5, 131.0, 47.6, 134, 0.25, "F.Cu", 54),
    seg(41.9, 126, 41.9, 123.8, 0.25, "F.Cu", 54),
    via(41.9, 123.8, 54),
    seg(41.9, 123.8, 43, 113.5, 0.25, "B.Cu", 54),
    via(43, 113.5, 54),
);
...
r.push(
    ...
    seg(38.4, 134, 38.4, 131.5, 0.22, "F.Cu", 59),
    via(38.4, 131.5, 59),
    seg(38.4, 131.5, 38.4, 119.675, 0.22, "B.Cu", 59),
    seg(38.4, 119.675, 32.4, 119.675, 0.22, "B.Cu", 59),
    via(32.4, 119.675, 59),
    seg(32.4, 119.675, 34.8, 119.675, 0.22, "F.Cu", 59),
    seg(29.1, 134, 29.1, 131.5, 0.22, "F.Cu", 58),
    via(29.1, 131.5, 58),
    seg(29.1, 131.5, 29.1, 120.325, 0.22, "B.Cu", 58),
    seg(29.1, 120.325, 30.5, 120.325, 0.22, "B.Cu", 58),
    via(30.5, 120.325, 58),
    seg(30.5, 120.325, 31, 120.325, 0.22, "F.Cu", 58),
    seg(31, 120.325, 31, 121.5, 0.22, "F.Cu", 58),
    seg(31, 121.5, 33.8, 121.5, 0.22, "F.Cu", 58),
    seg(33.8, 121.5, 33.8, 120.325, 0.22, "F.Cu", 58),
    seg(33.8, 120.325, 34.8, 120.325, 0.22, "F.Cu", 58),
);

// 65 SHDN has its own short B.Cu lane to R39.
r.push(
    seg(39.2, 120.975, 40.2, 122.8, 0.22, "F.Cu", 65),
    via(40.2, 122.8, 65),
    seg(40.2, 122.8, 44.1, 126, 0.22, "B.Cu", 65),
    via(44.1, 126, 65),
);

// 55 GATE distribution uses the very top of the extension, away from BAT divider/sense lanes.
r.push(
    seg(39.2, 119.025, 40.5, 116.5, 0.25, "F.Cu", 55),
    via(40.5, 116.5, 55),
    seg(40.5, 116.5, 40.5, 110, 0.25, "B.Cu", 55),
    seg(40.5, 110, 64, 110, 0.25, "B.Cu", 55),
    seg(47, 110, 47, 118, 0.25, "B.Cu", 55),
    via(47, 118, 55),
    seg(47, 118, 49, 118, 0.25, "F.Cu", 55),
    seg(64, 110, 64, 118, 0.25, "B.Cu", 55),
    via(64, 118, 55),
    seg(64, 118, 62.16, 118, 0.25, "F.Cu", 55),
);
...
r.push(
    seg(39.2, 119.675, 40.2, 120.0, 0.22, "F.Cu", 57),
    via(40.2, 120.0, 57),
    seg(40.2, 120.0, 40.2, 139, 0.22, "B.Cu", 57),
    seg(40.2, 139, 59.62, 139, 0.22, "B.Cu", 57),
    seg(59.62, 139, 59.62, 118, 0.22, "B.Cu", 57),
    seg(59.62, 118, 59.62, 124, 0.9, "F.Cu", 57),
    seg(59.62, 124, 66.5, 124, 0.9, "F.Cu", 57),
    seg(66.5, 124, 66.5, 126, 0.6, "F.Cu", 57),
    seg(66.5, 124, 73.1, 124, 0.6, "F.Cu", 57),
    seg(73.1, 124, 73.1, 126, 0.5, "F.Cu", 57),
    seg(73.1, 124, 73.8, 124, 0.5, "F.Cu", 57),
    seg(73.8, 124, 73.8, 126.25, 0.5, "F.Cu", 57),
    seg(73.8, 125.25, 76.1, 125.25, 0.35, "F.Cu", 57),
    seg(73.8, 125.75, 76.1, 125.75, 0.35, "F.Cu", 57),
    seg(73.8, 126.25, 76.1, 126.25, 0.35, "F.Cu", 57),
    via(70, 130, 57),
    seg(66.5, 124, 70, 130, 0.5, "F.Cu", 57),
    seg(70, 130, 88.9, 130, 0.5, "B.Cu", 57),
    via(88.9, 130, 57),
    seg(88.9, 130, 88.9, 126, 0.5, "F.Cu", 57),
    seg(88.9, 126, 87, 126, 0.5, "F.Cu", 57),
    seg(87, 124.25, 87, 126.25, 0.5, "F.Cu", 57),
    seg(79.9, 125.25, 87, 125.25, 0.35, "F.Cu", 57),
    seg(79.9, 125.75, 87, 125.75, 0.35, "F.Cu", 57),
    seg(79.9, 126.25, 87, 126.25, 0.35, "F.Cu", 57),
    seg(84.6, 124.25, 87, 124.25, 0.25, "F.Cu", 57),
);

// 60 EN and 61 RT now terminate immediately at their resistors.
r.push(seg(79.9, 124.25, 82.4, 124.25, 0.18, "F.Cu", 60));
r.push(
    seg(79.9, 123.75, 81.2, 122.7, 0.18, "F.Cu", 61),
    seg(81.2, 122.7, 82.4, 122, 0.18, "F.Cu", 61),
);
...
r.push(
    seg(78.25, 123.1, 78.25, 119.5, 0.18, "F.Cu", 63),
    seg(78.25, 119.5, 78.9, 118, 0.18, "F.Cu", 63),
);
r.push(
    seg(78.75, 123.1, 79.7, 121.0, 0.22, "F.Cu", 3),
    via(79.7, 121.0, 3),
    seg(77.75, 123.1, 76.7, 121.0, 0.22, "F.Cu", 3),
    via(76.7, 121.0, 3),
    seg(81.1, 118, 82.2, 117.0, 0.25, "F.Cu", 3),
    via(82.2, 117.0, 3),
);
...
r.push(
    ...
    seg(74.9, 116, 74.9, 118.5, 0.18, "F.Cu", 62),
    via(74.9, 118.5, 62),
    seg(74.9, 118.5, 75.0, 121.0, 0.18, "B.Cu", 62),
    via(75.0, 121.0, 62),
    seg(75.0, 121.0, 76.75, 123.1, 0.18, "F.Cu", 62),
    seg(77.1, 116, 78.3, 116, 0.25, "F.Cu", 3),
    via(78.3, 116, 3),
);

// Regulated +5 to R33/C15 and BIAS uses a far-right B.Cu trunk, never through C10 or Pi GND pads.
r.push(
    seg(64.9, 112, 63.5, 112, 0.22, "F.Cu", 2),
    via(63.5, 112, 2),
    seg(70.9, 112, 69.5, 112, 0.22, "F.Cu", 2),
    via(69.5, 112, 2),
    seg(63.5, 112, 63.5, 109, 0.3, "B.Cu", 2),
    seg(69.5, 112, 69.5, 109, 0.3, "B.Cu", 2),
    seg(63.5, 109, 94, 109, 0.4, "B.Cu", 2),
    seg(69.5, 109, 94, 109, 0.4, "B.Cu", 2),
    seg(76.1, 123.75, 74.2, 122.7, 0.22, "F.Cu", 2),
    via(74.2, 122.7, 2),
    seg(74.2, 122.7, 94, 122.7, 0.3, "B.Cu", 2),
    seg(94, 109, 94, 138, 0.5, "B.Cu", 2),
    via(94, 138, 2, 1.1, 0.55),
);

// U5 side GND pins and exposed pads.  Vias are offset away from the VIN rails.
r.push(
    seg(76.1, 127.25, 74.1, 127.25, 0.25, "F.Cu", 3),
    via(74.1, 127.25, 3),
    seg(76.1, 127.75, 74.1, 128.0, 0.25, "F.Cu", 3),
    via(74.1, 128.0, 3),
    seg(76.1, 128.25, 74.1, 128.75, 0.25, "F.Cu", 3),
    via(74.1, 128.75, 3),
    seg(79.9, 127.25, 82.5, 127.25, 0.25, "F.Cu", 3),
    via(82.5, 127.25, 3),
    seg(79.9, 127.75, 82.5, 128.0, 0.25, "F.Cu", 3),
    via(82.5, 128.0, 3),
    seg(79.9, 128.25, 82.5, 128.75, 0.25, "F.Cu", 3),
    via(82.5, 128.75, 3),
    via(77.3375, 124.45, 3, 0.8, 0.4),
    via(78.6625, 124.45, 3, 0.8, 0.4),
    via(77.3375, 126, 3, 0.8, 0.4),
    via(78.6625, 126, 3, 0.8, 0.4),
    via(77.3375, 127.55, 3, 0.8, 0.4),
    via(78.6625, 127.55, 3, 0.8, 0.4),
);

// Local input capacitor grounds.
r.push(
    seg(69.5, 126, 68.5, 128, 0.3, "F.Cu", 3),
    via(68.5, 128, 3),
    seg(70.9, 126, 70.9, 128, 0.3, "F.Cu", 3),
    via(70.9, 128, 3),
);
...
for (const x of [77.25, 77.75, 78.25, 78.75, 79.25])
    r.push(seg(x, 128.9, x, 131.2, 0.18, "F.Cu", 66));
r.push(
    seg(77.25, 131.2, 79.25, 131.2, 0.8, "F.Cu", 66),
    seg(78.25, 131.2, 78, 135, 1.4, "F.Cu", 66),
);
...
r.push(
    seg(100.5, 143, 100.5, 146, 0.4, "F.Cu", 3),
    via(100.5, 146, 3, 1.0, 0.5),
    seg(105.5, 143, 105.5, 146, 0.4, "F.Cu", 3),
    via(105.5, 146, 3, 1.0, 0.5),
);

// 15 servo signal follows the extreme left edge.  64 servo power goes around the header GND pin.
r.push(
    seg(8, 80, 5, 82, 0.22, "F.Cu", 15),
    via(5, 82, 15),
    seg(5, 82, 5, 143, 0.22, "B.Cu", 15),
    via(5, 143, 15),
    seg(5, 143, 6, 145, 0.22, "F.Cu", 15),
);
r.push(
    via(16.2, 149, 2),
    seg(16.2, 149, 16.2, 145, 0.7, "F.Cu", 2),
...
r.push(
    seg(30.4, 126, 31.0, 126, 0.35, "F.Cu", 3),
    via(31.0, 126, 3),
    seg(26.4, 134, 25.0, 134, 0.25, "F.Cu", 3),
    via(25.0, 134, 3),
    seg(34.8, 120.975, 33.5, 123.0, 0.25, "F.Cu", 3),
    via(33.5, 123.0, 3),
);

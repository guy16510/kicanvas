...
execFileSync(process.execPath, ["scripts/route-esp32-carrier-v3-drc3.mjs"], {
    stdio: "inherit",
});
...
function coords(text) {
    return [
        ...text.matchAll(/\((?:start|end|at)\s+([-\d.]+)\s+([-\d.]+)/g),
...
for (const token of ["(segment", "(via"]) {
    for (const e of blocks(token)) {
        ...
        const custom =
            [54, 55, 57, 65].includes(n) ||
            ([2, 3].includes(n) && pts.some(([, y]) => y >= 108));
        if (custom) deletions.push(e);
    }
}
for (const e of deletions.sort((a, b) => b.start - a.start))
...
r.push(
    ...
    seg(46.5, 113.5, 46.5, 134, 0.3, "F.Cu", 54),
    seg(46.5, 134, 47.6, 134, 0.3, "F.Cu", 54),
    seg(46.5, 132, 44, 132, 0.3, "F.Cu", 54),
    seg(44, 132, 41.9, 130, 0.3, "F.Cu", 54),
);

// Shutdown remains F.Cu and never shares the BAT_FUSED branch corridor.
r.push(
    seg(39.2, 120.975, 45.5, 120.975, 0.25, "F.Cu", 65),
    seg(45.5, 120.975, 45.5, 130, 0.25, "F.Cu", 65),
...
r.push(
    ...
    via(40, 136, 55),
    seg(40, 136, 58, 136, 0.25, "F.Cu", 55),
    via(58, 136, 55),
    seg(58, 136, 67, 136, 0.25, "B.Cu", 55),
    seg(49, 136, 49, 118, 0.25, "B.Cu", 55),
    seg(67, 136, 67, 116, 0.25, "B.Cu", 55),
    seg(67, 116, 62.16, 116, 0.25, "B.Cu", 55),
    seg(62.16, 116, 62.16, 118, 0.25, "B.Cu", 55),
);

// LTC4367 protected-output sense. First via is well clear of NC pin 6.
r.push(
    seg(39.2, 119.675, 41.5, 119.675, 0.25, "F.Cu", 57),
    via(41.5, 119.675, 57),
    seg(41.5, 119.675, 41.5, 145, 0.25, "B.Cu", 57),
    seg(41.5, 145, 59.62, 145, 0.25, "B.Cu", 57),
    seg(59.62, 145, 59.62, 118, 0.35, "B.Cu", 57),
    via(59.62, 118, 57, 1.2, 0.6),
);
...
r.push(
    ...
    seg(59.62, 124, 61, 128, 0.6, "F.Cu", 57),
    seg(61, 128, 64, 132, 0.6, "F.Cu", 57),
    seg(66.5, 124, 66.5, 126, 0.4, "F.Cu", 57),
    seg(66.5, 126, 66.5, 123, 0.3, "F.Cu", 57),
    seg(66.5, 123, 73.1, 123, 0.3, "F.Cu", 57),
    seg(73.1, 123, 73.1, 126, 0.3, "F.Cu", 57),
    seg(73.1, 126, 74.2, 126, 0.3, "F.Cu", 57),
    seg(74.2, 125.25, 74.2, 126.25, 0.3, "F.Cu", 57),
    seg(74.2, 125.25, 76.1, 125.25, 0.3, "F.Cu", 57),
    seg(74.2, 125.75, 76.1, 125.75, 0.3, "F.Cu", 57),
    seg(74.2, 126.25, 76.1, 126.25, 0.3, "F.Cu", 57),
);

// Right-side VIN feed drops above the FET, traverses B.Cu at y=115, then
// approaches the U5 VIN bank from above. Stopping the trunk at C13's VIN-side
// pad leaves the capacitor's ground-side pad clear for a direct plane return.
r.push(
    seg(59.62, 118, 59.62, 115, 0.7, "B.Cu", 57),
    seg(59.62, 115, 88.9, 115, 0.7, "B.Cu", 57),
    seg(88.9, 115, 88.9, 124, 0.5, "B.Cu", 57),
    via(88.9, 124, 57),
    seg(88.9, 124, 86.2, 124, 0.35, "F.Cu", 57),
    seg(86.2, 124, 82.9, 126, 0.35, "F.Cu", 57),
    seg(84.6, 124.25, 86.2, 124, 0.3, "F.Cu", 57),
    seg(83, 125.25, 83, 126.25, 0.3, "F.Cu", 57),
    seg(79.9, 125.25, 83, 125.25, 0.3, "F.Cu", 57),
    seg(79.9, 125.75, 83, 125.75, 0.3, "F.Cu", 57),
    seg(79.9, 126.25, 83, 126.25, 0.3, "F.Cu", 57),
);

// +5 V feedback endpoints join on B.Cu above the new power section.
r.push(
    seg(64.9, 112, 64.9, 110, 0.25, "F.Cu", 2),
    via(64.9, 110, 2),
    seg(64.9, 110, 64.9, 108, 0.3, "B.Cu", 2),
    seg(70.9, 112, 70.9, 110, 0.25, "F.Cu", 2),
    via(70.9, 110, 2),
...
r.push(
    seg(76.1, 123.75, 74, 123.75, 0.25, "F.Cu", 2),
    via(74, 123.75, 2),
    seg(74, 123.75, 74, 150, 0.3, "B.Cu", 2),
    seg(74, 150, 90, 150, 0.5, "B.Cu", 2),
    via(90, 150, 2, 1.1, 0.55),
);
...
r.push(
    via(16.2, 149, 2),
    seg(16.2, 149, 90, 149, 0.8, "B.Cu", 2),
    via(90, 149, 2, 1.1, 0.55),
    seg(16.2, 149, 16.2, 145, 0.7, "F.Cu", 2),
);

// U5 top grounds route away from SS before the vias, eliminating the NC/SS squeeze.
r.push(
    seg(78.75, 123.1, 78.75, 121.4, 0.25, "F.Cu", 3),
    seg(78.75, 121.4, 79.8, 121.4, 0.25, "F.Cu", 3),
    seg(79.8, 121.4, 79.8, 120.3, 0.25, "F.Cu", 3),
    via(79.8, 120.3, 3),
    seg(77.75, 123.1, 77.75, 121.4, 0.25, "F.Cu", 3),
    seg(77.75, 121.4, 76.7, 121.4, 0.25, "F.Cu", 3),
    seg(76.7, 121.4, 76.7, 120.3, 0.25, "F.Cu", 3),
    via(76.7, 120.3, 3),
);

// Side and exposed-pad grounds.
r.push(
    seg(76.1, 127.25, 74, 127.25, 0.25, "F.Cu", 3),
    via(74, 127.25, 3),
    seg(76.1, 127.75, 74, 128, 0.25, "F.Cu", 3),
    via(74, 128, 3),
    seg(76.1, 128.25, 74, 128.75, 0.25, "F.Cu", 3),
    via(74, 128.75, 3),
    seg(79.9, 127.25, 82.5, 127.25, 0.25, "F.Cu", 3),
    via(82.5, 127.25, 3),
    seg(79.9, 127.75, 82.5, 128, 0.25, "F.Cu", 3),
    via(82.5, 128, 3),
...
// Local grounds.
r.push(
    seg(69.5, 126, 69.5, 129, 0.3, "F.Cu", 3),
    via(69.5, 129, 3),
    seg(70.9, 126, 70.9, 129, 0.3, "F.Cu", 3),
    via(70.9, 129, 3),
    seg(91.1, 126, 91.1, 129, 0.3, "F.Cu", 3),
    via(91.1, 129, 3),
    seg(84.6, 122, 85.8, 122, 0.25, "F.Cu", 3),
    via(85.8, 122, 3),
    seg(77.1, 116, 78.5, 116, 0.25, "F.Cu", 3),
    via(78.5, 116, 3),
    seg(81.1, 118, 82.2, 118, 0.25, "F.Cu", 3),
    via(82.2, 118, 3),
    seg(30.4, 126, 31, 126, 0.35, "F.Cu", 3),
    via(31, 126, 3),
    seg(26.9, 134, 25.5, 134, 0.25, "F.Cu", 3),
    via(25.5, 134, 3),
    seg(34.8, 120.975, 33.5, 123, 0.25, "F.Cu", 3),
    via(33.5, 123, 3),
    seg(100.5, 143, 100.5, 146, 0.4, "F.Cu", 3),
    via(100.5, 146, 3, 1, 0.5),
    seg(105.5, 143, 105.5, 146, 0.4, "F.Cu", 3),
    via(105.5, 146, 3, 1, 0.5),
);
